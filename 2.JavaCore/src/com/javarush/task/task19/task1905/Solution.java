...
public class Solution {
    ...
    public static class DataAdapter implements RowItem{
        ...
        public String getContactFirstName() {
            return contact.getName().substring(contact.getName().trim().indexOf(" ")+1);
        }

        @Override
        public String getContactLastName() {
            return contact.getName().substring(0,contact.getName().trim().indexOf(" ")-1);
        }

        @Override
        public String getDialString() {
            String[] tel = contact.getPhoneNumber().split("\\(|\\)|-");
            String contact="callto://";
            for (String s: tel) {
                contact+=s;
            }
            return contact;
